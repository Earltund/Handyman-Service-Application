...
class ForgetPasswordFilledScreen extends StatelessWidget {ForgetPasswordFilledScreen({Key? key}) : super(key: key);

TextEditingController emailController = TextEditingController();

GlobalKey<FormState> _formKey = GlobalKey<FormState>();

@override Widget build(BuildContext context) { mediaQueryData = MediaQuery.of(context); return SafeArea(child: Scaffold(resizeToAvoidBottomInset: false, body: Form(key: _formKey, child: Container(width: double.maxFinite, padding: EdgeInsets.symmetric(horizontal: 23.h, vertical: 26.v), child: Column(children: [CustomImageView(imagePath: ImageConstant.imgSignal, height: 30.v, width: 49.h, onTap: () {onTapImgSignal(context);}), SizedBox(height: 46.v), Text("Forget Password", style: CustomTextStyles.titleMediumMedium), SizedBox(height: 9.v), Container(width: 257.h, margin: EdgeInsets.only(left: 43.h, right: 42.h), child: Text("Please provide the email address and we will send you an email with instructions on how to reset your password.", maxLines: 3, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: CustomTextStyles.bodySmallBluegray500)), SizedBox(height: 20.v), CustomFloatingTextField(controller: emailController, labelText: "Email", labelStyle: CustomTextStyles.labelLargeBlack900, hintText: "Email", textInputAction: TextInputAction.done, textInputType: TextInputType.emailAddress), SizedBox(height: 79.v), CustomElevatedButton(text: "Send", onPressed: () {onTapSend(context);}), SizedBox(height: 5.v)]))))); } 
...
/// Navigates to the enterOtpFilledScreen when the action is triggered.
onTapSend(BuildContext context) { Navigator.pushNamed(context, AppRoutes.enterOtpFilledScreen); } 
 }
